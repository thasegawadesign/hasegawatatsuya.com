# 長谷川 達也のポートフォリオ

<a href="https://www.hasegawatatsuya.com/" target="_blank">
  <img src="https://raw.githubusercontent.com/thasegawadesign/hasegawatatsuya.com/refs/heads/main/src/app/opengraph-image.jpg" alt="ポートフォリオを見る">
</a>

URL：https://www.hasegawatatsuya.com/

## テーマ

創造性を刺激する宇宙と調和する喜び

## コメント

有用で美しいものが「自在」につくれるようになることを目指して、Webについて勉強しています。
クリエイティブワーク、モノづくりの達人が目標です。

ホームページ制作の実務においては、想定される多様な訪問者のニーズに応えることを第一に考えます。
情報設計、UX/UIデザイン、コーディングの各段階でアクセシビリティ、審美性、セキュリティ、パフォーマンス、保守性などWebに求められる基本的な性質を多面的に配慮しつつ、商売道具としてのホームページが果たす役割に応えられるような安定した品質の成果物を確実に提供することが私の役割だと考えています。
さらに、ホームページ公開、運用開始後も継続的に改善を図り、まさにホームページを「育てる」姿勢で仕事に取り組みます。
